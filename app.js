...
// app.js — Sistema de Cardápio Oba Açaí Slz
// Versão FINAL — ✅ CORRIGIDO PARA FUNCIONAR COM O ADMIN (CATEGORIA COMO CAMPO) + AÇAÍ SEMPRE PRIMEIRO!
...
function mostrarPromptInstalacao() {
  ...
  promptDiv.innerHTML = `
    ...
    <button id="btn-cancelar-instalacao" style="
      background: #ccc;
      color: white;
      border: none;
      padding: 8px 16px;
      ...
      font-size: 14px;
    ">Cancelar</button>
    <button id="btn-instalar-app" style="
      background: #ea1d2c;
      color: white;
      border: none;
      padding: 8px 16px;
...
async function salvarPerfil() {
  ...
  if (!nome || !whatsapp || !endereco || !numero) {
    alert("⚠️ Nome, WhatsApp, Endereço e Número são obrigatórios!");
    return;
  }
  ...
  try {
    ...
    alert("Erro ao salvar no servidor. Verifique sua conexão ou permissões do Firebase.");
  }
}
...
function mostrarModal(titulo, mensagem) {
  ...
  // Conteúdo do modal
  box.innerHTML = `
    <h3 style="margin: 0 0 20px 0; color: var(--text-primary);">${titulo}</h3>
    <p style="margin: 0 0 25px 0; font-size: 1rem; color: var(--text-secondary); line-height: 1.5;">${mensagem}</p>
    <button class="btn-abrir-modal" onclick="fecharModal()" style="margin-top: 10px; padding: 10px 25px; font-size: 1rem;">
      OK
    </button>
  `;
  ...
  document.addEventListener('keydown', function fecharComEsc(e) {
    if (e.key === 'Escape') {
      fecharModal();
      document.removeEventListener('keydown', fecharComEsc); // Remove listener após uso
    }
  });
}
